...
public class KPainter extends Painter {
    ...
    private void paintChart() {
        ...
        for(int i = 0;i<mKDatas.size();i++){
            ...
            if(stock.TCLOSE > stock.TOPEN) mPaint.setColor(Color.RED);
            //if(stock.TOPEN < stock.LCLOSE) mPaint.setColor(Color.GREEN);
            if(stock.TCLOSE < stock.TOPEN) mPaint.setColor(Color.GREEN);
            if(stock.TOPEN == stock.TCLOSE) mPaint.setColor(Color.WHITE);
    ...
    private float getY(float price) {
        float result = (maxPrice - price) *((mHeight-10)/(this.maxPrice-this.minPrice));
        return result;
    }
    ...
    private float getVolumnY(int i) {
        float result = 0 - mKDatas.get(i).VOTURNOVER * (mHeight-10) / 3.0f / this.maxVolumn;
        return result;
    }
    ...
    public void setData(Object data){
        ...
        for(int i = 0;i<mKDatas.size();i++){
            minPrice = Math.min(mKDatas.get(i).TCLOSE*0.9f,minPrice);
            maxPrice = Math.max(mKDatas.get(i).TCLOSE*1.1f,maxPrice);
            maxVolumn = Math.max(mKDatas.get(i).VOTURNOVER*1.1f,maxVolumn);
        }
    }
}
